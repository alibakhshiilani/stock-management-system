...
const Form = ({onSubmit,defaultValues}) => {
  ...
  const { control, register, handleSubmit, formState: { errors } } = useForm({defaultValues:defaultValues || {}});
...
<label for="name" className="text-sm mb-2 block font-normal text-gray-500">شرح کالا</label>
...
<label for="expiration_date" className="text-sm mb-2 block font-normal text-gray-500">تاریخ انقضا</label>
  <Controller
          ...
          control={control}
          render={({ field }) => (
            <DatePicker className='w-full' inputClass="W-[100%] mb-4 p-4 block rounded-lg font-medium outline-none border border-gray-400 focus:border-green-500 focus:text-green-500 "
            onChange={(event) => {
              field.onChange(event?.value)
            }} defaultValue={defaultValues?.expiration_date ? new Date(defaultValues.expiration_date) : new Date()} value={field.value} inputAttributes={{placeholder:"تاریخ انقضا",style:{width:"100%"}}}/>
          )}
        />

    {
      errors?.["expiration_date"] && <span className='text-red-600 mb-5 block'>{errors?.["expiration_date"]?.message}</span>
    }   


<label for="login_date" className="text-sm mb-2 block font-normal text-gray-500">تاریخ ورود</label>
<input errors={errors} className='mb-4 border-gray-600 flex-auto p-4 block rounded-lg font-medium outline-none border focus:border-green-500 focus:text-green-500' name="login_date" value={moment().format("jYYYY/jM/jD")} disabled/>
    


    
      <label for="production_date" className="text-sm mb-2 block font-normal text-gray-500">تاریخ تولید</label>
  <Controller
          ...
          render={({ field }) => (
            ...
            onChange={(event) => {
              field.onChange(event?.value)
            }} defaultValue={defaultValues?.production_date ? new Date(defaultValues.production_date) : new Date()} value={field.value} inputAttributes={{placeholder:"تاریخ تولید",style:{width:"100%"}}}/>
          )}
        />

    {
      errors?.["production_date"] && <span className='text-red-600 mb-5 block'>{errors?.["production_date"]?.message}</span>
    }    
...
<select className='border-gray-500 w-full mb-3 flex-auto p-4 block rounded-lg font-medium outline-none border focus:border-green-500 focus:text-green-500 ' name="driver_id" 
  {...register("vehicle_id",{
